...
final class LocalModelDeviceTests:XCTestCase {
    @MainActor private func verify(_ id:String,thinking:String="off")async throws {
        ...
        runner.call(["model_id":id,"history":[["role":"user","content":prompt]],"thinking":thinking,"max_tokens":thinking=="off" ? 256:1024,"context_tokens":2048,"temperature":0.0]) {_,error in
        ...
        let finalAnswer=answer.components(separatedBy:"</think>").last ?? answer
        ...
        XCTAssertTrue(finalAnswer.contains("60")&&finalAnswer.contains("10"),"Check the supplied rates and difference: \(answer)")
        XCTAssertFalse(answer.contains("<turn|>"),"End-of-turn tokens must terminate generation")
    }
    @MainActor func testQwen2BThinkingEvidenceReasoning()async throws {try await verify("Qwen3.5-2B-4bit",thinking:"low")}
