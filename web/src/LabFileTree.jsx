...
export default function LabFileTree({files,selected,busy,onOpen,onNew,onClose}) {
 ...
 const source=files.filter(file=>!file.path.startsWith('runs/'));
 const tree=useMemo(()=>{
 ...
 const runs=files.filter(file=>file.path.startsWith('runs/')&&file.path.endsWith('/record.json')).slice(-25).reverse();
