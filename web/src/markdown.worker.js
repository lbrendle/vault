...
self.onmessage=({data})=>{try{
 if(data.kind==='prepare'){document=markdownSections(data.content);self.postMessage({kind:'prepared',sections:document.sections.map(({characters,lines,headings})=>({characters,lines,headings}))})}
