...
import {ArrowLeft,ArrowUp,Asterisk,Check,ChevronDown,Copy,FileText,Plus,Square,X} from 'lucide-react';
...
export default function LabAssistant({provider,onProvider,remote,onTarget,deviceLabel,model,onModel,file,includeFile,onIncludeFile,prompt,onPrompt,history,busy,edits,onEdits,onSend,onStop,onClose,onClear,onConnect}) {
 ...
 return <aside className="lab-assistant-pane" aria-label="Lab assistant">
  ...
  <div className="lab-assistant-transcript" ref={scroll} onScroll={()=>{const el=scroll.current;follow.current=el.scrollHeight-el.clientHeight-el.scrollTop<80;}}>
   {unsupported?<div className="lab-assistant-setup"><h3>Connect your Mac.</h3><p>Use {assistantName(provider)} with your Mac’s login while your notebook keeps running here.</p><button onClick={onConnect}>Connect Mac <ArrowUp size={14} style={{transform:'rotate(45deg)'}}/></button></div>:!history.length?<div className="lab-assistant-empty"><div className="lab-assistant-orbit" aria-hidden="true"><Asterisk size={32} strokeWidth={1}/></div><h3>Work through it together.</h3><p>Ask a question, explore an idea,<br/>or get help with your code.</p><div>{['Explain this file','Help me find a problem','Suggest a small test'].map(text=><button key={text} onClick={()=>suggestion(text)}>{text} <ArrowUp size={13} style={{transform:'rotate(45deg)'}}/></button>)}</div></div>:null}
