...
VAULT_APP="${VAULT_APP_OUTPUT:-$VAULT_PROJECT/dist/Vault.app}"
...
codesign --force --deep --options runtime --timestamp --sign "${VAULT_SIGNING_IDENTITY:--}" "$VAULT_STAGE/Vault.app"
if [ -e "$VAULT_APP" ]; then mv "$VAULT_APP" "$VAULT_STAGE/previous.app"; fi
mv "$VAULT_STAGE/Vault.app" "$VAULT_APP"
printf '%s\n' "$VAULT_APP"
